...
document.getElementById("encryption-form").addEventListener("submit", async (event) => {
  ...
  try {
    const payload = Object.fromEntries(new FormData(event.target).entries());
    const result = await api("/api/admin/encryption", { method: "POST", body: JSON.stringify(payload) }, adminSessionToken);
    event.target.reset();
    showToast(result.encryptionEnabled ? "데이터 암호화가 활성화되었습니다." : "데이터 암호화가 해제되었습니다.");
  } catch (error) {
    showToast(error.message, true);
  }
});
