...
def set_data_key(new_key: str) -> None:
    global CURRENT_DATA_KEY
    CURRENT_DATA_KEY = new_key.strip()
    if CURRENT_DATA_KEY:
        KEY_FILE.write_text(CURRENT_DATA_KEY, encoding="utf-8")
    else:
        KEY_FILE.unlink(missing_ok=True)


def initialize_data_key() -> None:
    env_key = os.environ.get(DATA_KEY_ENV, "").strip()
    if env_key:
        set_data_key(env_key)
        return

    if KEY_FILE.exists():
        set_data_key(KEY_FILE.read_text(encoding="utf-8").strip())
        return
...
class StudentSupportHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/admin/encryption":
            if not self.require_admin():
                return

            current_key = str(data.get("currentKey", "")).strip()
            new_key = str(data.get("newKey", "")).strip()

            if current_key != get_data_key():
                return self.json_response(400, {"error": "현재 암호화 키가 일치하지 않습니다."})

            with DB_LOCK:
                state = read_state()
                set_data_key(new_key)
                write_state(state)

            return self.json_response(200, {"ok": True, "encryptionEnabled": bool(get_data_key())})
